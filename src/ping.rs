...
use pnet::{
    ...
    packet::{
        ...
        icmpv6::{echo_reply::Icmpv6Codes, Icmpv6Code, Icmpv6Types, MutableIcmpv6Packet},
...
impl ICMPv6Ping {
    pub fn new() -> ICMPv6Ping {
        ...
        let scanner = ICMPv6Ping {
            interface: iface.clone(),
            gateway_mac: gateway_mac.parse().unwrap()
        };
        scanner
    }
    pub fn ping(&mut self, target: Ipv6Addr) {
        let source_ip = match self
            .interface
            .ips
            .iter()
        ...
        etherpacket.set_destination(self.gateway_mac);
        ...
        icmp_packet.set_payload("abcdefghijk".as_bytes());
